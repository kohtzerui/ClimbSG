export const SINGAPORE_EVENTS_2026 = [
  // January 
  {
    id: '2026-bp-benchmark-5',
    name: 'BP Benchmark 5',
    series: 'Boulder Planet',
    startDate: '2026-01-10',
    endDate: '2026-01-11',
    venue: 'Boulder Planet Sembawang',
    categories: ['FUN'],
    organiser: 'BoulderPlanet',
    notes: 'Get a nice pin to shower you can send BP5s!',
    regStatus: '**No registration available',
    logo: require('../assets/singapore2026logos/boulderplanetLogo.png')
  },
  {
      id: '2026-fitbloc-unbloc',
      name: 'Unbloc Final Evaluation',
      series: 'Fitbloc',
      startDate: '2026-01-31',
      endDate: '2026-02-01',
      venue: 'Fitbloc 3.0',
      categories: ['FINALS'],
      organiser: 'Fitbloc',
      notes: 'Location to be continued ;)',
      regStatus: '**No registration available',
      logo: require('../assets/singapore2026logos/fitblocLogo.png')
    },
  
  // February
    {
      id: '2026-bff-vengafest',
      name: 'BFF Vengafest',
      series: 'BFF',
      startDate: '2026-02-04',
      endDate: '2026-02-07',
      venue: 'BFF Bendemeer',
      categories: ['YOUTH A', 'YOUTH B', 'YOUTH C', 'YOUTH D', 'YOUTH E', 'YOUTH F', 'TEAM'],
      organiser: 'BFF Climbing',
      notes: 'Team (U13, Mixed Age, Veteran)',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/bffLogo.png')
    },
    {
      id: '2026-bp-single-mingler',
      name: 'BP Single Mingler',
      series: 'Boulder Planet',
      startDate: '2026-02-14',
      endDate: '2026-02-14',
      venue: 'Boulder Planet Sembawang',
      categories: ['FUN'],
      organiser: 'Boulder Planet',
      notes: 'Line up to MINGLE o.O',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/boulderplanetLogo.png')
    },
  
    // March
    {
      id: '2026-lighthouse-phoenix',
      name: 'Lighthouse Phoenix',
      series: 'Lighthouse',
      startDate: '2026-03-08',
      endDate: '2026-03-08',
      venue: 'Lighthouse',
      categories: ["CARNIVAL"],
      organiser: 'Lighthouse',
      notes: 'Women only Bouldering Comp - Novice & Intermediate',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/lighthouseLogo.png')
    },
    {
      id: '2026-bp-inter-varsity',
      name: 'BP Inter-Varsity Championship',
      series: 'Boulder Planet',
      startDate: '2026-03-14',
      endDate: '2026-03-14',
      venue: 'Boulder Planet Sembawang',
      categories: ["FINALS"],
      organiser: 'Boulder Planet',
      notes: 'Strongest Uni on the block!',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/boulderplanetLogo.png')
    },
  
    // April
    {
      id: '2026-bff-verticrux-speed',
      name: 'BFF Verticrux (Speed)',
      series: 'BFF',
      startDate: '2026-04-26',
      endDate: '2026-04-26',
      venue: 'BFF Tampines Hub',
      categories: ['YOUTH D', 'YOUTH E', 'YOUTH F', 'TEAM'],
      organiser: 'BFF Climbing',
      notes: 'Team (U17, General)',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/bffLogo.png')
    },
  
    // May
    {
      id: '2026-gu-defvity',
      name: 'GU Defvity',
      series: 'GU',
      startDate: '2026-05-16',
      endDate: '2026-05-17',
      venue: 'Ground Up Climbing Gym',
      categories: ['TEAMS', 'FUN'],
      organiser: 'Ground Up',
      notes: 'Auto belay Carnival (U13, Individual, Above 40)',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/groundupLogo.png')
    },  
    {
      id: '2026-bp-benchmark-6',
      name: 'BP Benchmark 6',
      series: 'Boulder Planet',
      startDate: '2026-05-16',
      endDate: '2026-05-17',
      venue: 'Boulder Planet Sembawang',
      categories: ['FUN'],
      organiser: 'BoulderPlanet',
      notes: 'Get a nice pin to shower you can send BP6s!',
      regStatus: 'No registration required',
      logo: require('../assets/singapore2026logos/boulderplanetLogo.png')
    },
  
    // June
    {
      id: '2026-transcend',
      name: 'Transcend 2026',
      series: 'Transcend',
      startDate: '2026-06-19',
      endDate: '2026-06-21',
      venue: 'HeartBeat@Bedok',
      categories: ['PROMOTIONAL', 'TEAM', 'SNBL'],
      organiser: 'NTU Climbing Team',
      notes: 'NOT CONFIRMED YET',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/transendLogo.png')
    },
    {
      id: '2026-bp-celebrations',
      name: 'BP Celebrations',
      series: 'Boulder Planet',
      startDate: '2026-06-19',
      endDate: '2026-06-21',
      venue: 'Boulder Planet Sembawang',
      categories: ['FUN'],
      organiser: '',
      notes: '',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/boulderplanetLogo.png')
    },
  
    // July
    {
      id: '2026-bp-benchmark-7',
      name: 'BP Benchmark 7',
      series: 'Boulder Planet',
      startDate: '2026-07-11',
      endDate: '2026-07-12',
      venue: 'Boulder Planet Sembawang',
      categories: ['FUN'],
      organiser: 'BoulderPlanet',
      notes: 'Get a nice pin to shower you can send BP7s!',
      regStatus: 'No registration required',
      logo: require('../assets/singapore2026logos/boulderplanetLogo.png')
    },
    {
      id: '2026-gu-klip',
      name: 'GU Klip',
      series: 'GU',
      startDate: '2026-07-17',
      endDate: '2026-07-19',
      venue: 'Ground Up Climbing Gym',
      categories: ['PROMOTIONAL','YOUTH A', 'YOUTH B', 'YOUTH C', 'YOUTH D', 'YOUTH E'],
      organiser: 'Ground Up',
      notes: 'Ground Up Lead Climbing Competition',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/groundupLogo.png')
    },
  
    // August
    {
      id: '2026-lighthouse-ignite',
      name: 'Lighthouse Ignite',
      series: 'Lighthouse',
      startDate: '2026-08-28',
      endDate: '2026-08-30',
      venue: 'Lighthouse',
      categories: ['SNBL', 'CARNIVAL'],
      organiser: 'Lighthouse',
      notes: 'Citizen (Under 25, 25-35, Above 35)',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/lighthouseLogo.png')
    },
  
    // September
    {
      id: '2026-bp-setter-day',
      name: 'BP Setter Day',
      series: 'Boulder Planet',
      startDate: '2026-09-26',
      endDate: '2026-09-26',
      venue: 'Boulder Planet Sembawang',
      categories: ['FUN'],
      organiser: 'Boulder Planet',
      notes: 'For aspiring route setters in Singapore',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/boulderplanetLogo.png')
    },
  
    // November
    {
      id: '2026-gu-gubc',
      name: 'GU Bouldering Cup',
      series: 'GU',
      startDate: '2026-11-13',
      endDate: '2026-11-15',
      venue: 'Ground Up Climbing Gym',
      categories: ['TEAM', 'YOUTH A', 'YOUTH B', 'YOUTH C', 'YOUTH D', 'YOUTH E'],
      organiser: 'Ground Up',
      notes: 'Calling for all Comp Kid Crushers! Masters (Above 35) also included.',
      regStatus: 'Registration is not open yet!', 
      logo: require('../assets/singapore2026logos/groundupLogo.png')
    },
    {
      id: '2026-bp-benchmark-8',
      name: 'BP Benchmark 8',
      series: 'Boulder Planet',
      startDate: '2026-11-14',
      endDate: '2026-11-15',
      venue: 'Boulder Planet Sembawang',
      categories: ['FUN'],
      organiser: 'Boulder Planet',
      notes: 'Get a nice pin to shower you can send BP8s!',
      regStatus: 'No registration required',
      logo: require('../assets/singapore2026logos/boulderplanetLogo.png')
    },
    {
      id: '2026-bff-mini-crusher',
      name: 'BFF Mini Crusher',
      series: 'BFF',
      startDate: '2026-11-14',
      endDate: '2026-11-15',
      venue: 'BFF Bendemeer',
      categories: ['FUN'],
      organiser: 'BFF Climbing',
      notes: 'U7, U8, Combines Boulder, Top Rope and Speed',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/bffLogo.png')
    },
  
    // December
    {
      id: '2026-boulderactive',
      name: 'BoulderActive 2026',
      series: 'BoulderActive',
      startDate: '2026-12-04',
      endDate: '2026-12-06',
      venue: 'City Square Mall',
      categories: ['PROMOTIONAL', 'TEAM'],
      organiser: 'NUS Climbing Team',
      notes: 'NOT CONFIRMED',
      regStatus: 'Registration is not open yet!',
      logo: require('../assets/singapore2026logos/boulderactiveLogo.png')
    }
  ]
  